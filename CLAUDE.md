...
Build progress: unit 1 (provider interface, `src/types/provider.ts`) is done; unit 2 (Gmail OAuth, `src/providers/gmail/auth.ts`, with token encryption in `src/crypto/` and the user store in `src/db/`) is in progress. No `api/` entry points exist yet.
